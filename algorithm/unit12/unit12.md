...
### 3. 알고리즘 동작 과정(Kahn's Algorithm)

**진입 차수가 0인 모든 노드**를 큐에 삽입한다. 큐가 빌 때까지 다음을 반복한다: 큐에서 노드를 꺼내 결과에 추가하고, 해당 노드에서 나가는 **간선을 제거**한다(연결된 노드의 **진입 차수 감소**). 새롭게 **진입 차수가 0이 된 노드**를 큐에 삽입한다.
